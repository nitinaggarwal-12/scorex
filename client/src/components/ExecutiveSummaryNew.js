    ...
    const fetchResults = async () => {
      try {
        ...
        if (data.editedExecutiveSummary) {
          ...
          // Extract dynamic content from executiveSummary or overall summary (generated by AI based on user responses)
          const dynamicSummary = data.executiveSummary || data.overall?.summary || '';
          
          // Parse summary sections (AI generates structured markdown or text)
          let strategicSection = dynamicSummary;
          if (dynamicSummary.includes('## CRITICAL CONSTRAINTS')) {
            strategicSection = dynamicSummary.split(/##\s*CRITICAL CONSTRAINTS/i)[0].trim();
          } else if (dynamicSummary.includes('##')) {
            strategicSection = dynamicSummary.split(/##/)[0].trim() || dynamicSummary;
          }
          if (!strategicSection || strategicSection.length < 30) {
            strategicSection = `Executive analysis for ${data.assessmentInfo?.organizationName || 'your organization'}: comprehensive evaluation across ${data.assessmentInfo?.completedPillars || 6} core pillars reveals clear modernization milestones to advance from ${data.overall?.level?.level || data.overall?.level || 'current'} to target maturity.`;
          }

          let constraintsSection = '';
          const constraintsMatch = dynamicSummary.match(/##\s*(?:CRITICAL CONSTRAINTS|KEY CHALLENGES|CONSTRAINTS)[\s\S]*?(?=##|$)/i);
          if (constraintsMatch) {
            constraintsSection = constraintsMatch[0].replace(/##\s*(?:CRITICAL CONSTRAINTS|KEY CHALLENGES|CONSTRAINTS)[^\n]*/i, '').trim();
          } else if (data.painPointRecommendations && data.painPointRecommendations.length > 0) {
            constraintsSection = data.painPointRecommendations.slice(0, 4).map(p => `• ${p.painPoint || p.title || p.name || p}: ${p.impact || p.recommendation || ''}`).join('\n');
          } else {
            constraintsSection = 'Your assessment identified key architectural bottlenecks and operational dependencies limiting time-to-insight and governance agility.';
          }
          ...
          // Extract expected outcomes from summary or derive from businessImpact
          const outcomesMatch = dynamicSummary.match(/(?:\*\*Expected Business Outcomes:\*\*|##\s*Expected Business Outcomes|##\s*Strategic Outcomes)([\s\S]*?)(?:\*\*|##|$)/i);
          let expectedOutcomes = outcomesMatch 
          ...
          if (!expectedOutcomes || expectedOutcomes.length === 0) {
            if (data.businessImpact) {
              expectedOutcomes = [
                data.businessImpact.decisionSpeed?.driver ? `Decision Velocity: ${data.businessImpact.decisionSpeed.driver}` : null,
                data.businessImpact.costOptimization?.driver ? `Cost Optimization: ${data.businessImpact.costOptimization.driver}` : null,
                data.businessImpact.manualOverhead?.driver ? `Productivity: ${data.businessImpact.manualOverhead.driver}` : null
              ].filter(Boolean);
            }
          }
          if (!expectedOutcomes || expectedOutcomes.length === 0) {
            expectedOutcomes = [
              'Improved data platform reliability and zero-trust governance controls',
              'Sub-second time-to-insight through declarative real-time pipeline automation',
              'Substantial reduction in manual engineering overhead via serverless infrastructure',
              'Full regulatory compliance auditing and automated row/column level data protection'
            ];
          }
