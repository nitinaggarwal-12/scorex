...
// Professional Enterprise Platform brand colors
const COLORS = {
  primary: '#FF3621',      // Enterprise Platform Red
  secondary: '#00A972',    // Enterprise Platform Green
...
class ProfessionalPDFExporter {
  ...
  addHeader() {
    ...
    this.doc.text('Enterprise Platform', this.margin, 25);
  ...
  addCoverPage() {
    ...
    this.doc.setFont('helvetica', 'bold');
    this.doc.text('Enterprise Platform', this.pageWidth / 2, 55, { align: 'center' });
    ...
    this.doc.setTextColor(COLORS.mediumGray);
    this.doc.text(
      'Prepared by Enterprise Platform Technical Maturity Assessment Platform',
      this.pageWidth / 2,
      this.pageHeight - 30,
      { align: 'center' }
    );
  ...
  addExecutiveSummary() {
    ...
    if (!summaryText || typeof summaryText !== 'string' || summaryText === '{}' || summaryText === '[object Object]' || summaryText === 'null' || summaryText === 'undefined') {
      summaryText = 'This assessment provides a comprehensive evaluation of your Enterprise Platform technical maturity across six key pillars. ' +
  ...
  addMethodology() {
    ...
    const methodologyText = `This assessment evaluates your organization's Enterprise Platform technical maturity across six key pillars. Each pillar contains multiple dimensions with specific questions designed to assess current capabilities and future aspirations.
...
export const generateProfessionalReport = (results, assessmentInfo) => {
  try {
    ...
    const filename = `platform_Maturity_Assessment_${orgName}_${date}.pdf`;
