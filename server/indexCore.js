...
app.post('/api/assessment/:id/submit', async (req, res) => {
  try {
    ...
    // 1. Immediately mark status as submitted so the user is never blocked or timed out
    ...
    await assessmentRepo.update(id, initialPayload);
    console.log(`✅ Assessment ${id} submitted immediately & saved to database`);

    // 2. Respond to client immediately (0ms wait, no Axios 30s timeout)
    res.json({
      success: true,
      ...
      assessment: {
        ...
        completedCategories: initialPayload.completedCategories
      }
    });
    ...
    (async () => {
      try {
        ...
        if (Object.keys(updatePayload).length > 0) {
          ...
          console.log(`✅ Assessment ${id} background AI report & diagrams persisted successfully`);
        }
      } catch (aiErr) {
        console.warn('Gemini report background generation notice:', aiErr.message);
      }
    })();
  } catch (error) {
