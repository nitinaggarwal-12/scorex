...
router.post('/synthesize-act', async (req, res) => {
  try {
    const { chapter, persona, style, engine, customApiKey, customVoiceId, sliderConfig, language } = req.body;
    if (!chapter) {
      return res.status(400).json({ success: false, error: 'Chapter payload is required' });
    }

    const result = await audioNarrationService.synthesizeAct(chapter, persona, style, engine, customApiKey, customVoiceId, sliderConfig, language || 'en');
