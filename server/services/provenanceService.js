...
// Third-party attributions and population-level statistics are never derivable from a
// single customer's assessment. If prose names an analyst house or makes a claim about
// "N% of organizations", it needs a declared source or it does not ship.
const ATTRIBUTION_PATTERN = /\b(gartner|forrester|idc|mckinsey|bain|deloitte|accenture|venturebeat|harvard business review|hbr)\b/i;
const POPULATION_CLAIM_PATTERN = /\b(?:\d+(?:\.\d+)?\s?%|\d+(?:\.\d+)?x)\s+of\s+(?:\w+\s+){0,2}(?:organi[sz]ations|companies|enterprises|firms|projects|teams|customers|respondents)\b|\bthe\s+average\s+\w+\s+(?:is|costs?|reaches)\b|\bon\s+average\b|\bindustry\s+benchmark\b/i;
...
function neutralizeGeneratedText(text, { externalSources = [], allowAssessmentNumbers = false } = {}) {
  ...
  // An analyst attribution or population statistic is unsupported even when it carries no
  // number the quantitative pattern recognizes (e.g. "the average GDPR fine is 2.7M EUR").
  if (!textContainsUnsupportedQuantification(text)
    && !ATTRIBUTION_PATTERN.test(text)
    && !POPULATION_CLAIM_PATTERN.test(text)) {
    return text;
  }
  ...
  // Analyst attributions and population statistics require a declared source regardless
  // of whether assessment-derived numbers are otherwise permitted in this context.
  if (ATTRIBUTION_PATTERN.test(text) || POPULATION_CLAIM_PATTERN.test(text)) {
    return ROI_CLAIM_PATTERN.test(text)
      ? 'Requires customer baseline data and explicit scenario assumptions before financial impact can be quantified.'
